...
def initialize_legally(distanceMatrix: np.ndarray):
    ...
    while i != length:
        if time.time() - start_time > 2.0: # Don't spend more than 1 second initializing one individual
            ...
            return initialize_legally(distanceMatrix)
        order = np.negative(np.ones((length), dtype=int))
...
def initializePopulation(distanceMatrix, lamb, percent=0.1):
    population = np.zeros((lamb, len(distanceMatrix)), dtype=int)
    greedy = int(lamb * percent)
    random = lamb - greedy
    for i in range(random):
        population[i] = initialize_legally(distanceMatrix)
    for i in range(greedy):
        population[i + random] = initialize_greedily(distanceMatrix)
    return population
...
class r0123456:
    ...
    def sharedElimination(self, population, offspring):
        ...
        for i in range(self.p.lamb):
            ...
            if i == 0:
                ...
                random_combined = combined[np.random.choice(combined.shape[0],int(combined.shape[0]/4),replace=False)]
                fvals = self.sharedFitnessWrapper(random_combined, survivors[0:i,:],betaInit=1)   
                idx = np.argmin(fvals)
    ...
    def optimize(self, filename):
        ...
        while(self.convergenceTest()):
            ...
            ### Normalize the starting points of individuals before recombination ###
            for i in range(self.p.lamb):
                population[i] = standardise(population[i])
...
if __name__ == "__main__":
    ...
    wandb.config.lambStart = 25
    wandb.config.lambEnd = 25
